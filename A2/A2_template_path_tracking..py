...
def define_terminal_cost_and_constraints(opti, X, S, w_p, x_init, c_path, r_path, w_final):
...
def create_and_solve_ocp(N, nx, nq, lbx, ubx,
                         #dt, 
                         x_init,              c_path, r_path, w_v, w_a, w_w, w_final, w_p,
                         tau_min, tau_max):
    ...
    terminal_cost = define_terminal_cost_and_constraints(opti, X, S, w_p, x_init,c_path, r_path, w_final)
...
if __name__ == "__main__":
    ...
    plt.figure(figsize=(10, 4))
    plt.plot(tt[:-1], w_sol, label=f'w', alpha=0.7)
    plt.xlabel('Time [s]')
